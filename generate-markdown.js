...
const getParsedJSON = filename => {
  let rawFile = fs.readFileSync(filename);
...
const buildRowsFromParsedJSON = parsedJSON => {
  let md = "";

  // header row
  md += buildRow(["Company", "WFH", "Travel", "Visitors", "Events", "Updated", "Source"]);
  md += buildRow(["---", "---", "---", "---", "---", "---", "---"]);
...
const generateMarkdownFromFile = filename => {
  const parsed = getParsedJSON(filename);
  const built = buildRowsFromParsedJSON(parsed);
  return built;
};

const generatedMarkdown = generateMarkdownFromFile("companies.json");
